...
for (k in 1:length(cat.pr.inc.vec)) {
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      if (rbinom(1, 1, ifelse(0.14/(TA.gen.l/cat.pr.inc.vec[k]) > 1, 1, 0.14/(TA.gen.l/cat.pr.inc.vec[k]))) == 1) { # catastrophe
        cat.alpha <- estBetaParams(0.5, 0.05^2)$alpha
        cat.beta <- estBetaParams(0.5, 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      if (rbinom(1, 1, ifelse(0.14/(MR.gen.l/cat.pr.inc.vec[k]) > 1, 1, 0.14/(MR.gen.l/cat.pr.inc.vec[k]))) == 1) { # catastrophe
        cat.alpha <- estBetaParams(0.5, 0.05^2)$alpha
        cat.beta <- estBetaParams(0.5, 0.05^2)$beta
...
DP.auc <- sum(sum(DP.Q.ext.pr)/length(cat.pr.inc.vec))
PA.auc <- sum(sum(PA.Q.ext.pr)/length(cat.pr.inc.vec))
ZT.auc <- sum(sum(ZT.Q.ext.pr)/length(cat.pr.inc.vec))
PH.auc <- sum(sum(PH.Q.ext.pr)/length(cat.pr.inc.vec))
VU.auc <- sum(sum(VU.Q.ext.pr)/length(cat.pr.inc.vec))
PG.auc <- sum(sum(PG.Q.ext.pr)/length(cat.pr.inc.vec))
SS.auc <- sum(sum(SS.Q.ext.pr)/length(cat.pr.inc.vec))
PT.auc <- sum(sum(PT.Q.ext.pr)/length(cat.pr.inc.vec))
SO.auc <- sum(sum(SO.Q.ext.pr)/length(cat.pr.inc.vec))
MN.auc <- sum(sum(MN.Q.ext.pr)/length(cat.pr.inc.vec))
OR.auc <- sum(sum(OR.Q.ext.pr)/length(cat.pr.inc.vec))
NR.auc <- sum(sum(NR.Q.ext.pr)/length(cat.pr.inc.vec))
GN.auc <- sum(sum(GN.Q.ext.pr)/length(cat.pr.inc.vec))
DN.auc <- sum(sum(DN.Q.ext.pr)/length(cat.pr.inc.vec))
AL.auc <- sum(sum(AL.Q.ext.pr)/length(cat.pr.inc.vec))
TC.auc <- sum(sum(TC.Q.ext.pr)/length(cat.pr.inc.vec))
TH.auc <- sum(sum(TH.Q.ext.pr)/length(cat.pr.inc.vec))
SH.auc <- sum(sum(SH.Q.ext.pr)/length(cat.pr.inc.vec))
DM.auc <- sum(sum(DM.Q.ext.pr)/length(cat.pr.inc.vec))
TA.auc <- sum(sum(TA.Q.ext.pr)/length(cat.pr.inc.vec))
MR.auc <- sum(sum(MR.Q.ext.pr)/length(cat.pr.inc.vec))
...
Qextpr.dat <- data.frame(cat.pr.inc.vec,DP.Q.ext.pr,PA.Q.ext.pr,ZT.Q.ext.pr,PH.Q.ext.pr,VU.Q.ext.pr,PG.Q.ext.pr,SS.Q.ext.pr,PT.Q.ext.pr,SO.Q.ext.pr,MN.Q.ext.pr,OR.Q.ext.pr,NR.Q.ext.pr,GN.Q.ext.pr,DN.Q.ext.pr,AL.Q.ext.pr,TC.Q.ext.pr,TH.Q.ext.pr,SH.Q.ext.pr,DM.Q.ext.pr,TA.Q.ext.pr,MR.Q.ext.pr)
colnames(Qextpr.dat) <- c("catinc",labs.vec)
dat.out <- data.frame(spp.mass.vec, Qext.auc)
colnames(dat.out) <- c("M","AUC")
rownames(dat.out) <- labs.vec
write.table(Qextpr.dat, file="catincQpr.csv", sep=",", dec = ".", row.names = F, col.names = T)
write.table(dat.out, file="catincout.csv", sep=",", dec = ".", row.names = T, col.names = T)

save.image("catinc.RData")
