...
######################################################################################
### *** SCENARIO 6 must also have been run prior to running this scenario, and ***  ##
### *** 'catincQpr.csv' (generated by SCENARIO 6) must be in the same directory *** ##
######################################################################################

####################################################################
## SCENARIO 7: PROGRESSIVELY INCREASE CATASTROPHIC MORTALITY RATE ##
####################################################################
...
# import generational divider limits for number of catastrophes (from previous simulation)
catincQpr <- read.csv("catincQpr.csv")
catincQpr1 <- catincQpr[,-1]
catincQpr2 <- ifelse(round(catincQpr1, 2) == 1, 1, 0)

max.geninc <- rep(NA,dim(catincQpr2)[2])
for (c in 1:dim(catincQpr2)[2]) {
  first1.sub <- which(catincQpr2[,c] == 1)[1]
  max.geninc[c] <- ifelse(is.na(first1.sub) == T, dim(catincQpr)[1], catincQpr[first1.sub,1])
}
max.geninc

cat.inc.vec <- seq(0.5, 0.05, -0.02)
DP.Q.ext.pr <- PA.Q.ext.pr <- ZT.Q.ext.pr <-  PH.Q.ext.pr <- VU.Q.ext.pr <- PG.Q.ext.pr <- SS.Q.ext.pr <- PT.Q.ext.pr <- SO.Q.ext.pr <- MN.Q.ext.pr <- OR.Q.ext.pr <- NR.Q.ext.pr <- GN.Q.ext.pr <- DN.Q.ext.pr <- AL.Q.ext.pr <- TC.Q.ext.pr <- TH.Q.ext.pr <- SH.Q.ext.pr <- DM.Q.ext.pr <- TA.Q.ext.pr <- MR.Q.ext.pr <- rep(NA,length(cat.inc.vec))

for (k in 1:length(cat.inc.vec)) {
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(DP.gen.l/sample(1:max.geninc[1],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(PA.gen.l/sample(1:max.geninc[2],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(ZT.gen.l/sample(1:max.geninc[3],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(PH.gen.l/sample(1:max.geninc[4],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(VU.gen.l/sample(1:max.geninc[5],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(PG.gen.l/sample(1:max.geninc[6],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(SS.gen.l/sample(1:max.geninc[7],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(PT.gen.l/sample(1:max.geninc[8],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(SO.gen.l/sample(1:max.geninc[9],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(MN.gen.l/sample(1:max.geninc[10],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(OR.gen.l/sample(1:max.geninc[11],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(NR.gen.l/sample(1:max.geninc[12],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(GN.gen.l/sample(1:max.geninc[13],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(DN.gen.l/sample(1:max.geninc[14],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(AL.gen.l/sample(1:max.geninc[15],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(TC.gen.l/sample(1:max.geninc[16],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(TH.gen.l/sample(1:max.geninc[17],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(SH.gen.l/sample(1:max.geninc[18],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(DM.gen.l/sample(1:max.geninc[19],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(TA.gen.l/sample(1:max.geninc[15],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      cpgen.adj <- 0.14/(MR.gen.l/sample(1:max.geninc[15],1))
      if (rbinom(1, 1, ifelse(cpgen.adj > 1, 1, cpgen.adj)) == 1) { # catastrophe
        cat.alpha <- estBetaParams(cat.inc.vec[k], 0.05^2)$alpha
        cat.beta <- estBetaParams(cat.inc.vec[k], 0.05^2)$beta
...
DP.auc <- sum(sum(DP.Q.ext.pr)/length(cat.inc.vec))
PA.auc <- sum(sum(PA.Q.ext.pr)/length(cat.inc.vec))
ZT.auc <- sum(sum(ZT.Q.ext.pr)/length(cat.inc.vec))
PH.auc <- sum(sum(PH.Q.ext.pr)/length(cat.inc.vec))
VU.auc <- sum(sum(VU.Q.ext.pr)/length(cat.inc.vec))
PG.auc <- sum(sum(PG.Q.ext.pr)/length(cat.inc.vec))
SS.auc <- sum(sum(SS.Q.ext.pr)/length(cat.inc.vec))
PT.auc <- sum(sum(PT.Q.ext.pr)/length(cat.inc.vec))
SO.auc <- sum(sum(SO.Q.ext.pr)/length(cat.inc.vec))
MN.auc <- sum(sum(MN.Q.ext.pr)/length(cat.inc.vec))
OR.auc <- sum(sum(OR.Q.ext.pr)/length(cat.inc.vec))
NR.auc <- sum(sum(NR.Q.ext.pr)/length(cat.inc.vec))
GN.auc <- sum(sum(GN.Q.ext.pr)/length(cat.inc.vec))
DN.auc <- sum(sum(DN.Q.ext.pr)/length(cat.inc.vec))
AL.auc <- sum(sum(AL.Q.ext.pr)/length(cat.inc.vec))
TC.auc <- sum(sum(TC.Q.ext.pr)/length(cat.inc.vec))
TH.auc <- sum(sum(TH.Q.ext.pr)/length(cat.inc.vec))
SH.auc <- sum(sum(SH.Q.ext.pr)/length(cat.inc.vec))
DM.auc <- sum(sum(DM.Q.ext.pr)/length(cat.inc.vec))
TA.auc <- sum(sum(TA.Q.ext.pr)/length(cat.inc.vec))
MR.auc <- sum(sum(MR.Q.ext.pr)/length(cat.inc.vec))
...
Qextpr.dat <- data.frame(1-cat.inc.vec,DP.Q.ext.pr,PA.Q.ext.pr,ZT.Q.ext.pr,PH.Q.ext.pr,VU.Q.ext.pr,PG.Q.ext.pr,SS.Q.ext.pr,PT.Q.ext.pr,SO.Q.ext.pr,MN.Q.ext.pr,OR.Q.ext.pr,NR.Q.ext.pr,GN.Q.ext.pr,DN.Q.ext.pr,AL.Q.ext.pr,TC.Q.ext.pr,TH.Q.ext.pr,SH.Q.ext.pr,DM.Q.ext.pr,TA.Q.ext.pr,MR.Q.ext.pr)
colnames(Qextpr.dat) <- c("catMinc",labs.vec)
dat.out <- data.frame(spp.mass.vec, Qext.auc)
colnames(dat.out) <- c("M","AUC")
rownames(dat.out) <- labs.vec
write.table(Qextpr.dat, file="catMincQpr.csv", sep=",", dec = ".", row.names = F, col.names = T)
write.table(dat.out, file="catMincout.csv", sep=",", dec = ".", row.names = T, col.names = T)

save.image("catMinc.RData")
