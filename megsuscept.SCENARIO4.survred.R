...
#####################################################################
## SCENARIO 4: PROGRESSIVELY REDUCE SURVIVAL ACROSS ALL AGE GROUPS ##
#####################################################################
...
surv.red.vec <- seq(0, 0.95, 0.025)
DP.Q.ext.pr <- PA.Q.ext.pr <- ZT.Q.ext.pr <-  PH.Q.ext.pr <- VU.Q.ext.pr <- PG.Q.ext.pr <- SS.Q.ext.pr <- PT.Q.ext.pr <- SO.Q.ext.pr <- MN.Q.ext.pr <- OR.Q.ext.pr <- NR.Q.ext.pr <- GN.Q.ext.pr <- DN.Q.ext.pr <- AL.Q.ext.pr <- TC.Q.ext.pr <- TH.Q.ext.pr <- SH.Q.ext.pr <- DM.Q.ext.pr <- TA.Q.ext.pr <- MR.Q.ext.pr <- rep(NA,length(surv.red.vec))

for (k in 1:length(surv.red.vec)) {
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      DP.s.stoch <- rbeta(length(DP.s.alpha), DP.s.alpha, DP.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      PA.s.stoch <- rbeta(length(PA.s.alpha), PA.s.alpha, PA.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      ZT.s.stoch <- rbeta(length(ZT.s.alpha), ZT.s.alpha, ZT.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      PH.s.stoch <- rbeta(length(PH.s.alpha), PH.s.alpha, PH.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      VU.s.stoch <- rbeta(length(VU.s.alpha), VU.s.alpha, VU.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      PG.s.stoch <- rbeta(length(PG.s.alpha), PG.s.alpha, PG.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      SS.s.stoch <- rbeta(length(SS.s.alpha), SS.s.alpha, SS.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      PT.s.stoch <- rbeta(length(PT.s.alpha), PT.s.alpha, PT.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      SO.s.stoch <- rbeta(length(SO.s.alpha), SO.s.alpha, SO.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      MN.s.stoch <- rbeta(length(MN.s.alpha), MN.s.alpha, MN.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      OR.s.stoch <- rbeta(length(OR.s.alpha), OR.s.alpha, OR.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      NR.s.stoch <- rbeta(length(NR.s.alpha), NR.s.alpha, NR.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      GN.s.stoch <- rbeta(length(GN.s.alpha), GN.s.alpha, GN.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      DN.s.stoch <- rbeta(length(DN.s.alpha), DN.s.alpha, DN.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      AL.s.stoch <- rbeta(length(AL.s.alpha), AL.s.alpha, AL.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      TC.s.stoch <- rbeta(length(TC.s.alpha), TC.s.alpha, TC.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      TH.s.stoch <- rbeta(length(TH.s.alpha), TH.s.alpha, TH.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      SH.s.stoch <- rbeta(length(SH.s.alpha), SH.s.alpha, SH.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      DM.s.stoch <- rbeta(length(DM.s.alpha), DM.s.alpha, DM.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      TA.s.stoch <- rbeta(length(TA.s.alpha), TA.s.alpha, TA.s.beta) * (1 - surv.red.vec[k])
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      MR.s.stoch <- rbeta(length(MR.s.alpha), MR.s.alpha, MR.s.beta) * (1 - surv.red.vec[k])
...
DP.auc <- sum(sum(DP.Q.ext.pr)/length(surv.red.vec))
PA.auc <- sum(sum(PA.Q.ext.pr)/length(surv.red.vec))
ZT.auc <- sum(sum(ZT.Q.ext.pr)/length(surv.red.vec))
PH.auc <- sum(sum(PH.Q.ext.pr)/length(surv.red.vec))
VU.auc <- sum(sum(VU.Q.ext.pr)/length(surv.red.vec))
PG.auc <- sum(sum(PG.Q.ext.pr)/length(surv.red.vec))
SS.auc <- sum(sum(SS.Q.ext.pr)/length(surv.red.vec))
PT.auc <- sum(sum(PT.Q.ext.pr)/length(surv.red.vec))
SO.auc <- sum(sum(SO.Q.ext.pr)/length(surv.red.vec))
MN.auc <- sum(sum(MN.Q.ext.pr)/length(surv.red.vec))
OR.auc <- sum(sum(OR.Q.ext.pr)/length(surv.red.vec))
NR.auc <- sum(sum(NR.Q.ext.pr)/length(surv.red.vec))
GN.auc <- sum(sum(GN.Q.ext.pr)/length(surv.red.vec))
DN.auc <- sum(sum(DN.Q.ext.pr)/length(surv.red.vec))
AL.auc <- sum(sum(AL.Q.ext.pr)/length(surv.red.vec))
TC.auc <- sum(sum(TC.Q.ext.pr)/length(surv.red.vec))
TH.auc <- sum(sum(TH.Q.ext.pr)/length(surv.red.vec))
SH.auc <- sum(sum(SH.Q.ext.pr)/length(surv.red.vec))
DM.auc <- sum(sum(DM.Q.ext.pr)/length(surv.red.vec))
TA.auc <- sum(sum(TA.Q.ext.pr)/length(surv.red.vec))
MR.auc <- sum(sum(MR.Q.ext.pr)/length(surv.red.vec))
...
Qextpr.dat <- data.frame(surv.red.vec,DP.Q.ext.pr,PA.Q.ext.pr,ZT.Q.ext.pr,PH.Q.ext.pr,VU.Q.ext.pr,PG.Q.ext.pr,SS.Q.ext.pr,PT.Q.ext.pr,SO.Q.ext.pr,MN.Q.ext.pr,OR.Q.ext.pr,NR.Q.ext.pr,GN.Q.ext.pr,DN.Q.ext.pr,AL.Q.ext.pr,TC.Q.ext.pr,TH.Q.ext.pr,SH.Q.ext.pr,DM.Q.ext.pr,TA.Q.ext.pr,MR.Q.ext.pr)
colnames(Qextpr.dat) <- c("Sred",labs.vec)
dat.out <- data.frame(spp.mass.vec, Qext.auc)
colnames(dat.out) <- c("M","AUC")
rownames(dat.out) <- labs.vec
write.table(Qextpr.dat, file="survredQpr.csv", sep=",", dec = ".", row.names = F, col.names = T)
write.table(dat.out, file="survredout.csv", sep=",", dec = ".", row.names = T, col.names = T)

save.image("survred.RData")
